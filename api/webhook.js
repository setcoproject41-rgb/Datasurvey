...
export default async function handler(req, res) {
  ...
  if (!chatId) {
    console.error("⚠️ chatId tidak ditemukan.");
    return res.status(400).send("Bad Request: chatId undefined");
  }
  ...
  // --------------------------- MENU LAPORAN ---------------------------
    ...
    const category = decodeURIComponent(
      callback_query.data.replace("cat_", "")
    );
  ...
  // --- kirim foto eviden ---
    ...
    if (!userState[chatId]?.folder_path) {
      return bot.sendMessage(
        chatId,
        "⚠️ Kirim /start ulang, sesi laporan tidak aktif."
      );
    }
  ...
  // --- kirim lokasi ---
  else if (message?.location && userState[chatId]) {
  ...
  // --- kirim keterangan ---
    ...
    message?.text &&
    userState[chatId]?.draft_id &&
    !["/start", "selesai"].includes(message.text.toLowerCase())
  ) {
  ...
  // --- konfirmasi kirim ---
    ...
    const data = userState[chatId];
    if (!data?.draft_id)
      return bot.sendMessage(chatId, "⚠️ Tidak ada draft aktif.");
  ...
  // --- tampilkan rekap laporan ---
    ...
    data.forEach((d) => {
      const material = d.nilai_material
        ? `Rp${Number(d.nilai_material).toLocaleString("id-ID")}`
        : "-";
      const jasa = d.nilai_jasa
        ? `Rp${Number(d.nilai_jasa).toLocaleString("id-ID")}`
        : "-";
      const total = d.total
  ...
  // --- tampilkan daftar designator dalam kategori ---
    ...
    const category = decodeURIComponent(
      callback_query.data.replace("info_cat_", "")
    );
    ...
    const buttons = designators.map((d) => [
      {
        text: d.designator,
        callback_data: `info_des_${encodeURIComponent(d.designator)}`,
      },
    ]);
  ...
  // --- tampilkan detail designator ---
    ...
    const designator = decodeURIComponent(
      callback_query.data.replace("info_des_", "")
    );
    ...
    const nilaiMaterial = data.nilai_material
      ? `Rp${Number(data.nilai_material).toLocaleString("id-ID")}`
      : "-";
    const nilaiJasa = data.nilai_jasa
      ? `Rp${Number(data.nilai_jasa).toLocaleString("id-ID")}`
      : "-";
...
🧰 Jasa       : ${nilaiJasa}
💰 Total      : Rp${Number(
      (data.nilai_material || 0) + (data.nilai_jasa || 0)
  ...
  // --- fallback ---
    ...
    await bot.sendMessage(
      chatId,
      "Perintah tidak dikenal. Gunakan /start untuk memulai."
    );
  }
