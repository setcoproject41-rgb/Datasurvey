...
bot.on("callback_query", async (query) => {
  ...
  try {
    ...
    if (data.startsWith("seg_")) {
      ...
      userState[chatId] = {
        ...(userState[chatId] || {}),
        segmentasi: seg.nama_segmentasi
      };
    ...
    if (data.startsWith("report_seg_")) {
      ...
      for (const r of rows) {
        msg += `🔧 *${r.designator}*\n📦 Material: ${
          r.nilai_material
            ? `Rp${Number(r.nilai_material).toLocaleString("id-ID")}`
            : "-"
        }\n🧰 Jasa: ${
          r.nilai_jasa ? `Rp${Number(r.nilai_jasa).toLocaleString("id-ID")}` : "-"
        }\n💰 Total: ${
          r.total ? `Rp${Number(r.total).toLocaleString("id-ID")}` : "-"
        }\n\n`;
      }
    ...
    if (data.startsWith("info_cat_")) {
      ...
      return bot.sendMessage(chatId, `Kategori: *${cat}*`, {
        parse_mode: "Markdown",
        reply_markup: { inline_keyboard: buttons }
      });
    }
    ...
    if (data.startsWith("info_des_")) {
      ...
      const mat = d.nilai_material
        ? `Rp${Number(d.nilai_material).toLocaleString("id-ID")}`
        : "-";
      const jas = d.nilai_jasa
        ? `Rp${Number(d.nilai_jasa).toLocaleString("id-ID")}`
        : "-";
      const total = `Rp${Number(
        (d.nilai_material || 0) + (d.nilai_jasa || 0)
...
/* ------------------------
   MESSAGE handlers (photo, location, text)
   ------------------------ */
  ...
  try {
    ...
    if (msg.photo && msg.photo.length) {
      ...
      if (!state?.folder_path && !state?.draft_id) {
        return bot.sendMessage(
          chatId,
          "⚠️ Tidak ada sesi aktif. Ketik /start lalu buat laporan."
        );
      }

      const folder = state.folder_path;
      if (!folder)
        return bot.sendMessage(chatId, "⚠️ Sesi tidak aktif. Pilih designator dulu.");
      ...
      const { error: uploadErr } = await supabase.storage
        .from("evidence")
        .upload(`${folder}/${fileName}`, Buffer.from(buf), {
      ...
      return bot.sendMessage(
        chatId,
        "📸 Foto berhasil diupload. Kirim foto lain atau ketik 'selesai'."
      );
    }
